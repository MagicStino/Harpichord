...
const ControlPanel: React.FC<ControlPanelProps> = ({ state, onChange, onReset }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-[#dcd0b8] rounded-[2.5rem] border-[4px] border-[#bdae93] shadow-[inset_0_4px_10px_rgba(0,0,0,0.1)] text-amber-950 font-black uppercase tracking-tight overflow-hidden">
        ...
        {(['MAIN', 'DRUMS', 'FX', 'SOUND'] as const).map(tab => (
          <button 
            ...
            className={`flex-1 py-3 text-[10px] font-black tracking-[0.1em] transition-all ${
              activeTab === tab ? 'bg-[#dcd0b8] text-amber-950 border-b-2 border-amber-800' : 'text-amber-900/40'
            }`}
          >
            {tab}
          </button>
        ...
        {activeTab === 'MAIN' ? (
          <>
            <div className="grid grid-cols-3 gap-1">
              ...
              <Knob label="HARP VOL" size="sm" value={state.harpVolume} onChange={(v) => onChange({ harpVolume: v })} />
              <Knob label="DRUMS VOL" size="sm" value={state.rhythmVolume} onChange={(v) => onChange({ rhythmVolume: v })} />
            </div>

            <div className="grid grid-cols-3 gap-1 bg-amber-900/5 p-3 rounded-2xl border border-amber-950/5">
               <Knob label="CHORD CUT" size="sm" color="amber-800" value={state.chordCutoff} onChange={(v) => onChange({ chordCutoff: v })} />
               <Knob label="HARP CUT" size="sm" color="amber-800" value={state.harpCutoff} onChange={(v) => onChange({ harpCutoff: v })} />
               <Knob label="DRUM CUT" size="sm" color="amber-800" value={state.rhythmCutoff} onChange={(v) => onChange({ rhythmCutoff: v })} />
            </div>

            <div className="bg-orange-950/5 p-4 rounded-3xl border border-orange-950/10 space-y-3">
              <div className="flex justify-between items-center">
                 <div className="flex flex-col">
                    <span className="text-[11px] font-black uppercase tracking-widest text-orange-900 leading-none">MASTER TUBE</span>
                 </div>
                 <button 
                  onClick={() => onChange({ tubeEnabled: !state.tubeEnabled })}
                  className={`w-12 h-6 rounded-full p-1 transition-all border border-orange-900/20 ${state.tubeEnabled ? 'bg-orange-600' : 'bg-gray-300'}`}
                >
              ...
              <div className="flex gap-1">
                {(['clean', 'soft', 'warm', 'hot'] as const).map(p => (
                  <button 
                    ...
                    className={`flex-1 py-1 rounded text-[9px] font-black border transition-all ${state.tubePreset === p ? 'bg-orange-600 text-white border-orange-800' : 'bg-orange-900/5 border-orange-900/10 hover:bg-orange-100'}`}
                  >
                    {p.toUpperCase()}
                  </button>
                ))}
            ...
            <div className="grid grid-cols-3 gap-1 bg-amber-900/5 p-3 rounded-2xl border border-amber-950/5 items-center">
                <Knob label="BASS VOL" size="sm" color="amber-700" value={state.bassVolume} onChange={(v) => onChange({ bassVolume: v })} />
                <button 
                  onClick={() => onChange({ bassEnabled: !state.bassEnabled })} 
                  className={`w-full h-10 rounded-xl border-2 shadow-sm transition-all flex flex-col items-center justify-center ${state.bassEnabled ? 'bg-amber-800 border-amber-950 text-white' : 'bg-black/10 border-black/20 text-black/40'}`}
                >
                   <span className="text-[11px] font-black leading-none mb-0.5">BASS</span>
                   <span className="text-[9px] font-bold opacity-70 uppercase">{state.bassEnabled ? 'ON' : 'OFF'}</span>
                </button>
                <Knob label="SUSTAIN" size="sm" value={state.sustain} onChange={(v) => onChange({ sustain: v })} />
            </div>

            <div className="flex flex-col bg-amber-900/5 p-3 rounded-2xl border border-amber-950/5 gap-2">
                <div className="flex justify-between items-center px-2">
                    ...
                    <input 
                      ...
                      className="w-16 bg-black/15 text-amber-900 text-center py-1 rounded border border-amber-950/20 text-[13px] font-black outline-none focus:ring-1 focus:ring-amber-800"
                    />
                </div>
          ...
          <div className="flex flex-col gap-3 py-1">
             <div className="grid grid-cols-2 gap-x-2 gap-y-3 justify-items-center">
                {Object.values(RhythmPattern).filter(p => p !== RhythmPattern.NONE).map(p => (
                  <button key={p} onClick={() => onChange({ rhythm: state.rhythm === p ? RhythmPattern.NONE : p })} className={`w-full py-2.5 rounded-xl border-2 text-[11px] font-black uppercase transition-all ${state.rhythm === p ? 'bg-amber-900 text-white border-amber-950 shadow-inner' : 'bg-[#eee3ce] border-[#bdae93]'}`}>
                    {p}
                  </button>
                ))}
             </div>
            ...
            <div className="bg-[#1a1a1a] p-4 rounded-3xl border-2 border-cyan-500/50 space-y-4">
              <div className="flex flex-col gap-2 border-b border-cyan-500/20 pb-2">
                 ...
                 <div className="grid grid-cols-5 gap-1">
                    {['1/4', '1/4D', '1/8', '1/8D', '1/8T', '1/16', '1/16D', '1/16T', '1/3', '1/5'].map(div => (
                      <button 
                        key={div} 
                        onClick={() => onChange({ delayDivision: div as DelayDivision })} 
                        className={`px-1 py-1 rounded text-[9px] font-black border transition-all ${state.delayDivision === div ? 'bg-cyan-500 text-black border-cyan-400' : 'text-cyan-500 border-cyan-900/30'}`}
                      >
                        {div}
                      </button>
                    ))}
          ...
          <div className="flex flex-col gap-6 py-2">
             <div className="bg-[#1a1a1a] p-5 rounded-3xl border-2 border-amber-600/30 space-y-4">
                <div className="space-y-2">
                   <h3 className="text-[12px] text-amber-600/80 tracking-widest text-center uppercase font-black">OSC WAVEFORMS</h3>
                   <div className="flex gap-4">
                      <div className="flex-1 flex flex-col gap-1">
                        <span className="text-[10px] text-center opacity-40 font-black">CHORD</span>
                        <div className="grid grid-cols-1 gap-1">
                          {['sine', 'triangle', 'square', 'sawtooth'].map(w => (
                            <button key={w} onClick={() => onChange({ chordWaveform: w as WaveformType })} className={`py-1.5 rounded text-[10px] font-black border transition-all ${state.chordWaveform === w ? 'bg-amber-600 text-black border-amber-400' : 'text-amber-800 border-amber-900/40 hover:text-amber-500'}`}>{w}</button>
                          ))}
                        </div>
                      </div>
                      <div className="flex-1 flex flex-col gap-1">
                        <span className="text-[10px] text-center opacity-40 font-black">HARP</span>
                        <div className="grid grid-cols-1 gap-1">
                          {['sine', 'triangle', 'square', 'sawtooth'].map(w => (
                            <button key={w} onClick={() => onChange({ harpWaveform: w as WaveformType })} className={`py-1.5 rounded text-[10px] font-black border transition-all ${state.harpWaveform === w ? 'bg-amber-600 text-black border-amber-400' : 'text-amber-800 border-amber-900/40 hover:text-amber-500'}`}>{w}</button>
                          ))}
                        </div>
                      </div>
                   </div>
